<div align="center">

# Hi, I'm Aniket Shahi

### 🛡️ Cybersecurity Enthusiast | Networking & IT Security | Security Labs & Projects

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Aniket%20Shahi-blue?style=for-the-badge\&logo=linkedin)](https://www.linkedin.com/in/aniket-shahi-126618375/)
[![GitHub](https://img.shields.io/badge/GitHub-shahianiket20-black?style=for-the-badge\&logo=github)](https://github.com/shahianiket20)
[![TryHackMe](https://img.shields.io/badge/TryHackMe-AniketSploit-red?style=for-the-badge\&logo=tryhackme)](https://tryhackme.com/p/AniketSploit)

</div>

---

## 🧑‍💻 About Me

* 🎓 BSc (Hons) Computer Networking & IT Security
* 🛡️ Interested in cybersecurity, ethical hacking and network security
* 🔍 Completed an Offensive Security internship at InRed Labs
* 🧪 Developing practical security skills through TryHackMe labs and hands-on projects
* 🐍 Building projects with Python, Flask and SQLite
* 🐧 Working with Linux and security tools such as Kali Linux, Nmap and Metasploit
* ☁️ Exploring AWS cloud technologies and cloud security
* 🚀 Focused on continuously improving my technical and problem-solving skills

---

## 🔐 Cybersecurity Focus

### 🛡️ Security

`Cybersecurity` `Ethical Hacking` `Endpoint Security` `Threat & Vulnerability Management`

### 🌐 Networking

`Subnetting` `Routing` `NAT` `ACL` `Network Security`

### 🐧 Security Tools & Platforms

`Kali Linux` `Nmap` `Metasploit` `Metasploitable 2` `TryHackMe`

### ☁️ Cloud

`AWS` `Cloud Security` `Cloud Operations` `Cloud Architecture`

---

## 💻 Technical Skills

### Programming

![Python](https://img.shields.io/badge/Python-3776AB?style=flat\&logo=python\&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat\&logo=openjdk\&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat\&logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat\&logo=css3\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat\&logo=javascript\&logoColor=black)

### Development & Databases

![Flask](https://img.shields.io/badge/Flask-000000?style=flat\&logo=flask\&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat\&logo=sqlite\&logoColor=white)

### Security & Networking

![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat\&logo=linux\&logoColor=black)
![Kali Linux](https://img.shields.io/badge/Kali%20Linux-557C94?style=flat\&logo=kalilinux\&logoColor=white)
![Nmap](https://img.shields.io/badge/Nmap-004170?style=flat)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=flat)

---

## 🚀 Featured Projects

### 🕵️ Cowrie Honeypot Attack Monitoring Dashboard

A cybersecurity monitoring project designed to detect and monitor unauthorized access attempts using a Cowrie SSH/Telnet honeypot.

**Technologies:**
`Cowrie` `Python` `Flask` `SQLite` `HTML` `CSS` `JavaScript` `Chart.js` `Ubuntu`

🔗 **Repository:** [View Project](https://github.com/shahianiket20/cowrie-honeypot-dashboard)

---

### 🔐 Smart Door Lock System

An IoT-based security project designed around a secure keypad-controlled door locking system.

**Technologies:**
`ESP32` `Arduino` `4x4 Keypad` `LCD` `Relay Module` `Solenoid Lock`

---

### 💻 Furniture Store Billing System

A Python-based console application for managing furniture inventory, sales and order-related updates using a text file.

**Technologies:**
`Python` `File Handling`

---

### ☕ GUI-Based Calculator

A simple Java Swing application capable of performing basic arithmetic operations through a graphical user interface.

**Technologies:**
`Java` `Swing`

---

## 🧪 TryHackMe

I use TryHackMe to develop practical cybersecurity skills through hands-on security labs and challenges.

🔗 **TryHackMe Profile:**
https://tryhackme.com/p/AniketSploit

---

## 📜 Certifications & Training

* **Cisco Networking Academy — Endpoint Security**
* **Cisco Networking Academy — Cyber Threat Management**
* **Cisco Networking Academy — Introduction to Cybersecurity**
* **AWS Academy — Cloud Security Foundations**
* **AWS Academy — Cloud Operations**
* **AWS Academy — Cloud Foundations**
* **AWS Academy — Cloud Architecting**
* **Google Cloud Skills Boost — Introduction to Generative AI**
* **Google Cloud Skills Boost — Introduction to Large Language Models**


---

## 🤝 Connect With Me

📧 **Email:** [shahianiket21@gmail.com](mailto:shahianiket21@gmail.com)

💼 **LinkedIn:**
https://www.linkedin.com/in/aniket-shahi-126618375/

🧪 **TryHackMe:**
https://tryhackme.com/p/AniketSploit

💻 **GitHub:**
https://github.com/shahianiket20

---

<div align="center">

### 🚀 Learning. Building. Securing.

</div>
